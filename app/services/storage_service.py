...
def crop_object_key(run_id, filename):
    """
...
def crop_path(run_id, filename):
    """
    Return the local filesystem path for a crop image.
    """

    return CROP_ROOT / crop_object_key(run_id, filename)


def crop_url(run_id, filename):
    """
    Return the HTTP URL path for a crop image.
    """

    return f"/crops/{crop_object_key(run_id, filename)}"


def ensure_crop_dir(run_id):
    """
    Ensure the local crop directory exists for a run.
    """

    directory = CROP_ROOT / run_id
